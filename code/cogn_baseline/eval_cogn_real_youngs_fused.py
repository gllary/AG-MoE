...
def find_weight_file(fold_dir: Path) -> Path | None:
...
def run_inference(fold_dirs: list[Path], x, batch_size: int) -> tuple[np.ndarray, list[np.ndarray]]:
    ...
    for fold_dir in fold_dirs:
        weight_path = find_weight_file(fold_dir)
...
def build_metric_rows(score_name: str, y_true: np.ndarray, y_pred: np.ndarray, ks: list[int], deltas: list[float], n_boot: int, seed: int):
    rows = []
    for k in ks:
...
def save_predictions(out_dir: Path, y_true: np.ndarray, score_k: np.ndarray, score_g: np.ndarray, score_fused: np.ndarray, all_k: list[np.ndarray], all_g: list[np.ndarray]):
    preds = []
    ...
    payload = {
        ...
        "score_k_ensemble": score_k.tolist(),
        "score_g_ensemble": score_g.tolist(),
        "score_fused_ensemble": score_fused.tolist(),
        "score_k_per_fold": [p.tolist() for p in all_k],
        "score_g_per_fold": [p.tolist() for p in all_g],
    }
    with open(out_dir / "predictions_full.json", "w") as f:
        json.dump(payload, f, indent=2)


def save_topk_table(out_dir: Path, y_true: np.ndarray, y_pred: np.ndarray, topk: int, name: str):
    import pandas as pd
    ...
    df.sort_values("rank_pred").to_csv(out_dir / f"top{topk}_hit_table_{name}.csv", index=False)


def main():
    ap = argparse.ArgumentParser(description="Evaluate dual coNGN checkpoints and compute fused_KG on real Young's modulus datasets.")
    ap.add_argument("--weights_k", required=True, help="Fold dir or run dir for the log_kvrh coNGN model.")
    ap.add_argument("--weights_g", required=True, help="Fold dir or run dir for the log_gvrh coNGN model.")
    ap.add_argument(
        "--data_json",
        required=True,
    ...
    ap.add_argument("--out_dir", default="cogn_real_eval_fused")
    args = ap.parse_args()

    weights_k = Path(args.weights_k)
    weights_g = Path(args.weights_g)
    ...
    print("=" * 60)
    print("coNGN real-data fused_KG evaluation")
    print(f"[Eval] weights_k = {weights_k}")
    print(f"[Eval] weights_g = {weights_g}")
    ...
    fold_dirs_k = resolve_fold_dirs(weights_k)
    fold_dirs_g = resolve_fold_dirs(weights_g)
    print(f"[Eval] using {len(fold_dirs_k)} K fold dir(s)")
    print(f"[Eval] using {len(fold_dirs_g)} G fold dir(s)")

    dataset, y_true = load_real_dataset(data_json)
    x = get_model_inputs(dataset)

    score_k, all_k = run_inference(fold_dirs_k, x, args.batch_size)
    score_g, all_g = run_inference(fold_dirs_g, x, args.batch_size)
    score_fused = fusion_softplus_phys(score_k, score_g)

    rows = []
    rows.extend(build_metric_rows("cogn_log_kvrh", y_true, score_k, ks, deltas, args.n_boot, args.seed))
    rows.extend(build_metric_rows("cogn_log_gvrh", y_true, score_g, ks, deltas, args.n_boot, args.seed))
    rows.extend(build_metric_rows("fused_KG", y_true, score_fused, ks, deltas, args.n_boot, args.seed))
    save_metrics(out_dir, rows)
    save_predictions(out_dir, y_true, score_k, score_g, score_fused, all_k, all_g)
    save_topk_table(out_dir, y_true, score_k, args.topk_table, "cogn_log_kvrh")
    save_topk_table(out_dir, y_true, score_g, args.topk_table, "cogn_log_gvrh")
    save_topk_table(out_dir, y_true, score_fused, args.topk_table, "fused_KG")

    run_cfg = {
        "weights_k": str(weights_k),
        "weights_g": str(weights_g),
        "fold_dirs_k": [str(p) for p in fold_dirs_k],
        "fold_dirs_g": [str(p) for p in fold_dirs_g],
        "data_json": str(data_json),
        "n_samples": int(len(y_true)),
        "fusion": "softplus_phys: 9KG / (3K + G)",
    }
    ...
    print(f"  {out_dir / f'top{args.topk_table}_hit_table_fused_KG.csv'}")
